        ...
        for line_num in transcribe_lines:
            ...
            if 'transcribe(' in line:
                ...
                if line.strip().endswith(')'):
                    # Insert VAD parameters before the closing parenthesis
                    new_line = line.replace(')', ', vad_filter=True, vad_parameters={"threshold": 0.5})')
                    lines[idx] = new_line
                    modified = True
                else:
                    ...
                    while search_idx < len(lines) and paren_count > 0:
                        ...
                        if paren_count == 0:
                            ...
                            if search_line.strip() == ')':
                                ...
                                # Closing paren with other content
                                lines[search_idx] = search_line.replace(')', ', vad_filter=True, vad_parameters={"threshold": 0.5})')
                            modified = True
                            break
                        search_idx += 1
